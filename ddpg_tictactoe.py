...
parser.add_argument('--exploration_noise', default=0.8, type=float)
    ...
    if args.mode == 'test':
        ...
        for i in range(args.test_iteration):
            ...
            for t in count():
                ...
                next_state, reward, done, trunc, info = env.step(np.float32(action[0]))
                ep_r += reward
                env.render()
                if done or t >= 10:
        ...
        for i in range(args.max_episode):
            ...
            env.render()
            total_step += step+1
            print("Total T:{} Episode: \t{}/{} Total Reward: \t{}".format(total_step, i,args.max_episode, total_reward))
            agent.update()
