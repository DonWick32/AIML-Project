...
class ReplayBuffer(object):
    ...
    def store(self, s, a, r, s_, dw):
        # print("=======")
        # print(s, a, r, s_, dw)
        # print(self.s, self.a, self.r, self.s_, self.dw)
        # print(self.s, s)
        self.s[self.count] = s
        self.a[self.count] = a
        self.r[self.count] = r
...
class DDPG(object):
    ...
    def learn(self, relay_buffer):
        ...
        current_Q = self.critic(batch_s, batch_a.unsqueeze(-1))
...
def evaluate_policy(env, agent):
    ...
    for _ in range(times):
        ...
        while (not done) and (not trunc):
            a = agent.choose_action(s)[0]  # We do not add noise when evaluating
            if a<0:
                a = np.float64(0)
...
if __name__ == '__main__':
    ...
    env_evaluate = TicTacToeEnv(size=3)  # When evaluating the policy, we need to rebuild an environment
    number = 1
    # Set random seed
    seed = 0
    # # env.seed(seed)
    ...
    action_dim = 1
    max_action = 8
    max_episode_steps = 9 # Maximum number of steps per episode
    ...
    # Build a tensorboard
    writer = SummaryWriter(log_dir='DDPG_env_{}_number_{}_seed_{}'.format(env_name[env_index], number, seed))

    noise_std = 0.1 * max_action  # the std of Gaussian noise for exploration
    max_train_steps = 5e5  # Maximum number of training steps
    random_steps = 5e2  # Take the random actions in the beginning for the better exploration
    ...
    pgbar = tqdm(total=max_train_steps)
    ...
    while total_steps < max_train_steps:
        ...
        while (not done) and (not trunc) :
            ...
            # a = torch.clamp(torch.tensor(a), 0, max_action).numpy()
            if a<0:
                a = np.float64(0)
            # print(a, type(a))
            x = env.step(a)
            s_, r, done, trunc, _ = x
            # print(x, env.steps_left)
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                evaluate_num += 1
                # print("Evaluate_num:{}".format(evaluate_num))
                ...
                if evaluate_num % 10 == 0:
                    np.save('DDPG_env_{}_number_{}_seed_{}.npy'.format(env_name[env_index], number, seed), np.array(evaluate_rewards))
