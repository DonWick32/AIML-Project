    ...
    if args.mode == 'test':
        ...
        for i in range(args.test_iteration):
            ...
            for t in count():
                ...
                next_state, reward, done, trunc, info = env.step(np.float32(action))
                ep_r += reward
                env.render()
        ...
        for i in range(args.max_episode):
            ...
            for t in count():
                action = agent.select_action(state)
                action = (action + np.random.normal(0, args.exploration_noise, size=env.observation_space.shape[0])).clip(
